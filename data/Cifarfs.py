...
class Cifarfs_DataLoader(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.train_dataset = CifarfsDataset(root_dir=' ', transform=train_transform, N=self.n_way, K=self.k_shot, Q=self.q_query, is_train=True) 
        
        self.test_dataset = CifarfsDataset(root_dir=' ', transform=test_transform, N=self.n_way, K=self.k_shot, Q=self.q_query, is_train=False) 
    ...
    #     batch_sampler_val = MetaBatchSampler(self.val_dataset.class_dict, self.n_way, self.k_shot, self.q_query,self.n_iter)
    #     val_loader = DataLoader(self.val_dataset,  batch_sampler=batch_sampler_val,num_workers=4)
    #     return val_loader 
...
if __name__ == '__main__':
    ...
    train_dataset = CifarfsDataset(root_dir, transform=train_transform, N=5, K=5, Q=15, is_train=True)
    print(train_dataset.class_names)
   # print(train_dataset.img_paths)
    loader = Cifarfs_DataLoader(data_dir=' ', n_way=5, k_shot=5, q_query=15, n_iter=10, transform=train_transform)
    loader.setup()
